...
async function saveSong() {
  ...
  const payload = {
    ...
    lyrics_links: makeLyricsSearchLinks(title, qs("#artistName").value.trim()),
  ...
  let { error } = await supabaseClient.from("songs").upsert(payload);
  if (error && /spotify_url|cover_art_url|lyrics_links|schema cache|column/i.test(error.message || "")) {
    const fallbackPayload = { ...payload };
    delete fallbackPayload.spotify_url;
    delete fallbackPayload.cover_art_url;
    delete fallbackPayload.lyrics_links;
    const retry = await supabaseClient.from("songs").upsert(fallbackPayload);
    error = retry.error;
    if (!error) toast("保存しました。追加カラム未作成のため、Spotify/ジャケット/歌詞リンクは詳細画面で自動生成します");
  }
